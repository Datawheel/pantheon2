...
class Person extends Component {
  ...
  render() {
    ...
    const maxPageViews =
      wikiPageViews && wikiPageViews.items
        ? Math.max(...wikiPageViews.items.map((d) => d.views || 0))
        : 0;
    ...
    if (newsArticles.length) {
      sections.push({
        title: "In the news",
        slug: "news_articles",
        content: <News person={person} newsArticles={newsArticles} />,
      });
    }
    ...
    sections.push({
      ...
      content: <OccupationRanking person={person} ranking={occupationRanks} />,
    });
    if (person.birthyear) {
      sections.push({
        ...
        content: (
          <YearRanking
            person={person}
            birthYearRanking={birthYearRanks}
            deathYearRanking={deathYearRanks}
          />
        ),
      });
    }
    if (person.bplace_country) {
      sections.push({
        ...
        slug: "country_peers",
        content: <CountryRanking person={person} ranking={birthCountryRanks} />,
      });
      sections.push({
        ...
        content: (
          <CountryOccupationRanking
            person={person}
            ranking={birthCountryOccupationRanks}
          />
        ),
      });
    }
    ...
    if (["ACTOR", "COMEDIAN"].includes(person.occupation.id)) {
      sections.push({
        title: "Television and Movie Roles",
        slug: "movies",
    ...
    const pageHeaderMetaTags = config.meta.map((meta) => {
      if (meta.property) {
        if (meta.property === "og:title") {
          titleContent =
            wikiSummary && wikiSummary.description
              ? `${person.name} Biography - ${wikiSummary.description}`
              : titleContent;
        ...
        if (meta.property === "og:image") {
          return {
            ...
            content: `${pageUrl
              .replace("http://", "https://")
              .replace("/profile/", "/images/screenshots/")}.jpg`,
          };
        }
        if (meta.property === "og:description") {
          if (wikiSummary && wikiSummary.description) {
            const s2 = `${
              person.gender ? (person.gender === "M" ? "His" : "Her") : "Their"
            } biography is available in ${
              person.l
            } different languages on Wikipedia making ${
              person.gender ? (person.gender === "M" ? "him" : "her") : "them"
            } the ${FORMATTERS.ordinal(
              person.occupation_rank_unique
            )} most popular ${person.occupation.occupation.toLowerCase()}.`;
            return {
              property: "og:description",
              content: `Pantheon profile of ${person.name}, ${wikiSummary.description}. ${s2}`,
            };
          }
        }
      }
    ...
    return (
      <div className="person">
        <Helmet title={titleContent} meta={pageHeaderMetaTags} />
...
const personURL =
  "/person?slug=eq.<id>&select=occupation(*),bplace_geonameid(*),bplace_country(*),dplace_geonameid(*),*";
...
const wikiURL =
  "https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exsentences=4&explaintext&exsectionformat=wiki&exintro&pageids=<person.id>&format=json&exlimit=1&origin=*";
...
const wikiSummaryURL =
  "https://en.wikipedia.org/api/rest_v1/page/summary/<person.wikiSlug>";
const wikiRelatedURL =
  "https://pantheon.world/api/wikiRelated?slug=<person.wikiSlug>";
...
Person.preneed = [
  fetchData("personRanks", personRanksURL, (res) => {
    ...
    const occupationRankLow = Math.max(
      1,
      parseInt(person.occupation_rank_unique, 10) - NUM_RANKINGS_PRE
    );
    const occupationRankHigh = Math.max(
      NUM_RANKINGS,
      parseInt(person.occupation_rank_unique, 10) + NUM_RANKINGS_POST
    );
    // Calculate min/max for birthyear peers
    const birthYearRank = person.birthyear_rank_unique;
    const birthYearRankLow = Math.max(
      1,
      parseInt(person.birthyear_rank_unique, 10) - NUM_RANKINGS_PRE
    );
    const birthYearRankHigh = Math.max(
      NUM_RANKINGS,
      parseInt(person.birthyear_rank_unique, 10) + NUM_RANKINGS_POST
    );
    // Calculate min/max for deathyear peers
    const deathYearRank = person.deathyear_rank_unique;
    const deathYearRankLow = person.deathyear_rank_unique
      ? Math.max(
          1,
          parseInt(person.deathyear_rank_unique, 10) - NUM_RANKINGS_PRE
        )
      : "9999";
    const deathYearRankHigh = person.deathyear_rank_unique
      ? Math.max(
          NUM_RANKINGS,
          parseInt(person.deathyear_rank_unique, 10) + NUM_RANKINGS_POST
        )
      : "9999";
    ...
    const bplaceCountryRankLow = person.bplace_country_rank_unique
      ? Math.max(
          1,
          parseInt(person.bplace_country_rank_unique, 10) - NUM_RANKINGS_PRE
        )
      : "9999";
    const bplaceCountryRankHigh = person.bplace_country_rank_unique
      ? Math.max(
          NUM_RANKINGS,
          parseInt(person.bplace_country_rank_unique, 10) + NUM_RANKINGS_POST
        )
      : "9999";
    // ensure birthcountry is non NULL
    const bplaceCountry = person.bplace_country || "";
    //
    const bplaceCountryOccupationRank =
      person.bplace_country_occupation_rank_unique;
    const bplaceCountryOccupationRankLow =
      person.bplace_country_occupation_rank_unique
        ? Math.max(
            1,
            parseInt(person.bplace_country_occupation_rank_unique, 10) -
              NUM_RANKINGS_PRE
          )
        : "9999";
...
Person.need = [
  ...
  fetchData(
    "birthCountryOccupationRanks",
    birthCountryOccupationRanksURL,
    (res) => res
  ),
