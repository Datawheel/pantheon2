...
class Yearbook extends Component {
  constructor(props) {
    ...
    this.sections = [
      ...
      {
        slug: "major-cities-by-number-of-births",
        title: "Major Cities By Number of Births",
      },
      {
        slug: "major-cities-by-number-of-deaths",
      ...
      if (meta.property) {
        ...
        if (meta.property === "og:description") {
          return {
            ...
            content: `The most globally famous people born in ${year} include ${peopleBornInYear[0].name}, ${peopleBornInYear[1].name}, ${peopleBornInYear[2].name}, ${peopleBornInYear[3].name} and ${peopleBornInYear[4].name}.`,
          };
        }
      }
      return meta;
...
Yearbook.need = [
  fetchData(
    "peopleBornInYear",
    "/person?select=name,l,l_,age,non_en_page_views,coefficient_of_variation,hpi,id,slug,gender,birthyear,deathyear,bplace_country(id,country,continent,slug),bplace_geonameid(id,place,country,slug,lat,lon),dplace_geonameid(id,place,country,slug),occupation_id:occupation,occupation(id,occupation,occupation_slug)&birthyear=eq.<year>&hpi=gte.4&order=hpi.desc.nullslast"
  ),
];
