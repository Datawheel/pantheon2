...
module.exports = function(app) {
  ...
  app.get("/api/movies", async(req, res) => {
    ...
    const personPantheonResp = await axios.get(`https://api.pantheon.world/person?id=eq.${id}&select=name,slug,occupation.occupation_name`).catch(e => (console.log(`[Movies API]: No Person in DB with id: ${id}`), {data: []}));
    ...
    if (person.occupation !== "ACTOR" && person.occupation !== "FILM DIRECTOR") return res.json([]);
    ...
    const moviesFromPantheonDbResp = await axios.get(`https://api.pantheon.world/movie?pid=eq.${id}&last_fetch=gte.${sixMonthsAgo}`).catch(e => (console.log("[Movies API] Error finding movie in db:", e), {data: []}));
    ...
    await axios.delete(`https://api.pantheon.world/movie?pid=eq.${id}`,
      {headers: {
    ...
    ).catch(e => (console.log("[Movies API] DELETE Error in db:", e), {data: []}));

    const movieDbPerson = await axios.get(`https://api.themoviedb.org/3/search/person?api_key=${apiKey}&language=en-US&query=${encodeURIComponent(person.name)}&page=1&include_adult=true`)
      .then(res => {
      ...
      .then(res => {
        ...
        if (res.data) {
          if (person.occupation.id === "FILM DIRECTOR" && res.data.crew) {
            ...
            const knownForMovies = res.data.cast.filter(d => movieIds.includes(d.id));
    ...
    const cleanedMovieData = movieData.map(movie => {
      ...
      return {
        ...
        role: roleData && roleData.character && roleData.character.trim() ? roleData.character : null,
        overview: movie.overview && movie.overview.trim() ? movie.overview : null,
        ...
        homepage: movie.homepage && movie.homepage.trim() ? movie.homepage : null
      };
    });

    // UPSERT via "Prefer: resolution=merge-duplicates" header
    const moviePosts = cleanedMovieData.map(movie => axios.post("https://api.pantheon.world/movie?columns=pid,slug,mid,title,poster,backdrop,role,overview,tagline,genres,release_date,average_rating,rating_count,budget,revenue,runtime,homepage", movie, {
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiZGVwbG95In0.Es95xLgTB1583Sxh8MvamXIE-xEV0QsNFlRFVOq_we8",
        "Prefer": "resolution=merge-duplicates"
      ...
      console.log(`[Movies API] unable to post movie by ${person.name} to db.`);
      console.log(err);
      return {error: err};
    }));
