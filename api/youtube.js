...
module.exports = function(app) {
  ...
  app.get("/api/youtube/:pid", async(req, res) => {
    ...
    const pantheonData = await axios.get(`https://api.pantheon.world/person?id=eq.${pid}&select=name,slug,occupation,youtube`, {
      headers: {
    ...
    const ytData = await axios.get(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${pantheonData.name}%20${pantheonData.occupation.occupation}&maxResults=1&type=video&videoEmbeddable=true&key=${ytApiKey}`)
      ...
      .catch(error => {
        errorMsg = error.response && error.response.data && error.response.data.error && error.response.data.error.errors && error.response.data.error.errors.length && error.response.data.error.errors[0].message ? error.response.data.error.errors[0].message : null;
        return null;
      });
    if (ytData && ytData.id && ytData.id.videoId) {
      const ytId = ytData.id.videoId;
      await axios.patch(`https://api.pantheon.world/person?id=eq.${pid}`, {youtube: ytId}, {
