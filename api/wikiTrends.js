...
module.exports = function(app) {
  ...
  app.get("/api/wikiTrendDetails", async(req, res) => {
    ...
    const monthAgoTrendFromDbResp = await axios.get(`https://api.pantheon.world/trend?date=gte.${year1monthAgo}-${month1monthAgo}-${day1monthAgo}&pid=eq.${wikiId}&rank_pantheon=lte.100`).catch(e => (console.log("Pantheon DB trends read Error:", e), {data: []}));
    ...
    if (monthAgoPvFromDbResp.data.length) {
      ...
      const availableLangsResp = await axios.get(availableLangsApi).catch(e => (console.log("Wiki Trending Error:", e), {error: "Wiki ID not found"}));
      ...
      if (availableLangsJson.query && availableLangsJson.query.pages) {
        ...
        if (personResult) {
          ...
          langlinks.unshift({"*": personResult.title, "lang": "en", "langname": "English", "url": `https://en.wikipedia.org/wiki/${personResult.title}`});
          // const langlinksLookup = langlinks.reduce((obj, d) => (obj[d.lang] = d, obj), {});
          const langReqs = langlinks.map(ll => `https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/${ll.lang}.wikipedia/all-access/all-agents/${encodeURIComponent(ll["*"])}/daily/${monthAgo}/${yesterday}`);
          ...
          const pageViews = await Promise.all(langReqs.map(url => throttle(async() => {
            const res = await axios.get(url).catch(() => ({data: {items: []}}));
            return res.data ? res.data.items : [];
          })));

          // filter out empty results
          const pageViewsWithData = pageViews
          ...
          enrichedPageViewsFlat = pageViewsFlat.map(pv => {
            ...
            const formattedDate = `${pv.timestamp.slice(0, 4)}-${pv.timestamp.slice(4, 6)}-${pv.timestamp.slice(6, 8)}`;
            return {
              pid: wikiId,
              date: formattedDate,
              slug: pv.article,
