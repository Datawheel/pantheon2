...
module.exports = function(app) {
  ...
  app.get("/api/trivia/getQuestions", async(req, res) => {
   ...
   // Template: PERSON was born in ...
    ...
    const wrongCountry1 = wrongCountries[Math.floor(Math.random() * (wrongCountries.length-1)) + 1];
    const wrongCountry2 = wrongCountries[Math.floor(Math.random() * (wrongCountries.length-1)) + 1];
    const wrongCountry3 = wrongCountries[Math.floor(Math.random() * (wrongCountries.length-1)) + 1];
    const q1 = {
      question: `${randomEZBio.name} was born  in ...`,
    ...
    const randomEZBioIndex2 = Math.floor(Math.random() * biosInEZOccupations.length);
    const randomEZBio2 = biosInEZOccupations.splice(randomEZBioIndex2, 1)[0]; // Pop the item from the array
    const wrongCountries2 = [...new Set(biosInEZOccupations.map(d => d.bplace_country))].filter(d => Boolean(d) && d !== randomEZBio.bplace_country);
    const wrongCountry12 = wrongCountries[Math.floor(Math.random() * (wrongCountries2.length-1)) + 1];
    const wrongCountry22 = wrongCountries[Math.floor(Math.random() * (wrongCountries2.length-1)) + 1];
    const wrongCountry32 = wrongCountries[Math.floor(Math.random() * (wrongCountries2.length-1)) + 1];
    const q3 = {
      question: `${randomEZBio2.name} was born  in ...`,
      answer_a: wrongCountry12,
      answer_b: wrongCountry22,
      answer_c: randomEZBio2.bplace_country,
      answer_d: wrongCountry32,
      correct_answer: "c"
    };
    console.log("q3");
    console.log(q3);
    ...
    // Question 1.
   // Template: PERSON was a ...
   const topBiosInOccupation2 = await axios.get("https://api.pantheon.world/person_ranks?occupation_rank=lt.7&select=id,name,occupation,birthyear,bplace_geonameid,bplace_country,deathyear,dplace_country").then(result => result.data).catch(e => (console.log("[trivia] Pantheon DB Error:", e), {data: []}));
    let deadBiosInRandomBio2Occupation = [];
    let randomBio2;
    const randomIndex2 = Math.floor(Math.random() * topBiosInOccupation2.length);
    randomBio2 = topBiosInOccupation2.splice(randomIndex2, 1)[0]; // Pop the item from the array
    deadBiosInRandomBio2Occupation = topBiosInOccupation2.filter(d => d.bplace_country === randomBio2.bplace_country);
    const q2 = {
      question: `${randomBio2.name} was a ...?`,
      answer_a: deadBiosInRandomBio2Occupation[0].occupation,
      answer_b: deadBiosInRandomBio2Occupation[2].occupation,
      answer_c: randomBio2.occupation,
      answer_d: deadBiosInRandomBio2Occupation[1].occupation,
      correct_answer: "c"
    };
  ...
  // Question 1.
  // Template: Where was INVENTOR JOHANNES GUTENBERG born?
    // const topBiosInOccupation = await axios.get("https://api.pantheon.world/person_ranks?occupation_rank=lt.7&select=id,name,occupation,birthyear,bplace_geonameid,bplace_country,deathyear,dplace_country").then(result => result.data).catch(e => (console.log("[trivia] Pantheon DB Error:", e), {data: []}));
    ...
    const shuffledQuestions = shuffleArray([q1, q2, q3])
      .map((q, i) => ({...q, id: i + 1}));
