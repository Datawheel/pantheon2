...
module.exports = function(app) {
  ...
  app.post('/api/createTriviaScore', async function (req, res) {
    ...
    const {user_id, game_share_id, answers, token} = req.body;
    ...
    await db.trivia_score.create({"user_id": user_id, "ip_hash": ip_hash, "game_share_id" :game_share_id, "answers": answers, "score_bot": score}).catch(err => {
      console.error(err);
      res.status(500).json({
        success: false
      });
