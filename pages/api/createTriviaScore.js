...
module.exports = function(app) {
  ...
  app.post('/api/createTriviaScore', async function (req, res) {
    ...
    if (scoreDB > - 1){
      await db.trivia_score.create({"user_id": user_id, "ip_hash": ip_hash, "game_share_id" :game_share_id,"q_id":answer.qid,"question_id":answer.quid,"current_answer_option":answer.ao,"current_answer":answer.at,"correct_answer_option":answer.cao,"score_bot": scoreDB}).catch(err => {
        console.error(err);
        res.status(500).json({
          success: false
        });
      ...
      await db.trivia_score.create({"user_id": user_id, "ip_hash": ip_hash, "game_share_id" :game_share_id,"q_id":answer.qid,"question_id":answer.quid,"current_answer_option":answer.ao,"current_answer":answer.at,"correct_answer_option":answer.cao,"score_bot": score}).catch(err => {
        console.error(err);
        res.status(500).json({
          success: false
        });
