...
/* Meta Information */
// const {name} = JSON.parse(shell.cat(path.join(__dirname, "package.json")));
...
async function start() {
  ...
  deps = deps.filter((d, i) => d.includes("@datawheel/canon-") && deps.indexOf(d) === i).map(d => {
  ...
  const watchFiles = [path.join(appPath, "style.yml"), ...storeFiles, ...cacheFiles, ...dbFiles, ...routerFiles];
