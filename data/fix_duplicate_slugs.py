...
for d in duplicate_places:
    cursor.execute("SELECT slug, country_code, id FROM place WHERE slug=%s ORDER BY num_born DESC nulls last OFFSET 1", (d,))
    ...
    for dh in dupes_here:
        ...
        if cursor.fetchone():
            new_slug = "{},_{}_{}".format(dh[0], dh[1], dh[2])
...
for d in duplicate_people:
    cursor.execute("SELECT slug, id FROM person WHERE slug=%s ORDER BY langs DESC nulls last OFFSET 1", (d,))
    dupes_here = cursor.fetchall()
    for dh in dupes_here:
