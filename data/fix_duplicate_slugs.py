...
for d in duplicate_places:
    cursor.execute("SELECT slug, country_code, wiki_id, id FROM place WHERE slug=%s ORDER BY num_born DESC OFFSET 1", (d,))
    ...
    for dh in dupes_here:
        ...
        cursor.execute("UPDATE place SET slug=%s WHERE id=%s;", (new_slug, dh[3]))
        conn.commit()
...
for d in duplicate_people:
    cursor.execute("SELECT slug, wiki_id, id FROM person WHERE slug=%s ORDER BY langs DESC OFFSET 1", (d,))
    dupes_here = cursor.fetchall()
    for dh in dupes_here:
        new_slug = "{}_{}".format(dh[0], dh[1])
        cursor.execute("UPDATE person SET slug=%s WHERE id=%s;", (new_slug, dh[2]))
        conn.commit()
