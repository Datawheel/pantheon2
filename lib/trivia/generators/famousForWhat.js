...
export default async function generateFamousForWhat(rng, difficulty = "medium") {
  ...
  const people = await safeFetchJson(
    `${BASE_API}/person?select=id,name,slug,occupation,famous_for,description,occupation(id,occupation,occupation_slug,domain)&id=in.(${candidateIds})&famous_for=not.is.null`,
    {},
    []
  );
