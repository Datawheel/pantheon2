...
export default async function generateWhoWroteIt(rng, difficulty = "medium") {
  ...
  for (const book of shuffled) {
    const person = await safeFetchJson(
      `${BASE_API}/person_ranks?select=id,name,slug,gender,occupation&id=eq.${book.pid}`,
      {},
      []
    );
    if (person.length > 0) {
  ...
  const writers = await safeFetchJson(
    `${BASE_API}/person_ranks?select=id,name,slug,gender,occupation&occupation=eq.WRITER&id=neq.${subjectPerson.id}&order=hpi.desc&limit=50`,
    {},
    []
  );
