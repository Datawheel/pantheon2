...
export default async function generateWhoCameFirst(rng, difficulty = "medium") {
  ...
  const people = await safeFetchJson(
    `${BASE_API}/person_ranks?select=id,name,slug,gender,birthyear,occupation,hpi&birthyear=not.is.null&hpi=gte.${minHpi}&order=hpi.desc&limit=300`,
    {},
    []
  );
