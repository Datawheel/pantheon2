...
export function parseRankingsSearchParams(searchParams, occupations = [], pageType = "rankings") {
  ...
  const years = SANITIZERS.years(getSearchParam(searchParams, "years"));
...
export function buildRankingsMetadata(exploreState, places, nestedOccupations) {
  ...
  if (exploreState.birthMonth && exploreState.birthDay) {
    qualifiers.push(`Born on ${formatMonthDay(exploreState.birthMonth, exploreState.birthDay)}`);
  }
  if (exploreState.onlyShowNew) {
