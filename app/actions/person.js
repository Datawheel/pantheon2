...
export function fetchPerson(store) {
  const prom = makePersonRequest('get', null, null, `/person?id=eq.${store["id"]}&select=occupation{*},birthcountry{*},birthplace{*},birthyear{*},*`);
  return {
...
export function fetchOccupationRanks(store) {
  const getOccRank = makePersonRequest('get', null, null, `/person_occupation_rank?person=eq.${store["id"]}&select=occupation{*},rank,rank_unique,langs`);
  const getOccRankPeers = getOccRank.then(function(occRankRes) {
        ...
        console.log("OccRank API:", `/person_occupation_rank?occupation=eq.${occId}&rank_unique=gte.${rankSub}&rank_unique=lte.${rankPlus}&select=occupation{*},person{*},langs,rank`)
        return makePersonRequest('get', null, null, `/person_occupation_rank?occupation=eq.${occId}&rank_unique=gte.${rankSub}&rank_unique=lte.${rankPlus}&select=occupation{*},person{*},langs,rank,rank_unique`);
    });
...
export function fetchBirthcountryRanks(store) {
  const getBCRank = makePersonRequest('get', null, null, `/person_birthcountry_rank?person=eq.${store["id"]}&select=birthcountry{*},rank,rank_unique,langs`);
...
export function fetchBirthyearRanks(store) {
  const getBYRank = makePersonRequest('get', null, null, `/person_birthyear_rank?person=eq.${store["id"]}&select=birthyear{*},rank,rank_unique,langs`);
  const getBYRankPeers = getBYRank.then(function(byRankRes) {
        ...
        const apiURL = `/person_birthyear_rank?birthyear=eq.${byId}&rank_unique=gte.${rankSub}&rank_unique=lte.${rankPlus}&select=birthyear{*},person{*},langs,rank,rank_unique`
