...
export function fetchPeople(store) {
  const getPeopleProm = makeOccupationRequest('get', null, null, `/occupation?occupation_slug=eq.${store.id}&select=id`).then(function(profIdRes) {
    ...
    return makeOccupationRequest("get", null, null, `/person?occupation=eq.${profId}&order=hpi.desc.nullslast&select=birthplace{id,name,slug},birthcountry{id,continent,country_name,name},deathcountry{id,continent,country_name,name},occupation{*},occupation_id:occupation,*`);
  });
