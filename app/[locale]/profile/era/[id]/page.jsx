...
async function getEra(eraId) {
  const res = await fetch(
    `https://api-dev.pantheon.world/era?slug=eq.${eraId}`,
    {
      method: "GET",
      headers: {
...
async function getPeopleBornInEra(startYear, endYear) {
  const res = await fetch(
    `https://api-dev.pantheon.world/person?birthyear=gte.${startYear}&birthyear=lte.${endYear}&order=hpi.desc.nullslast&select=bplace_geonameid(id,place,slug,lat,lon),bplace_country(id,continent,country_code,country,slug),occupation(*),occupation_id:occupation,*`
  );
  return res.json();
}
...
async function getPeopleDiedInEra(startYear, endYear) {
  const res = await fetch(
    `https://api-dev.pantheon.world/person?deathyear=gte.${startYear}&deathyear=lte.${endYear}&order=hpi.desc.nullslast&select=dplace_country(id,continent,country_code,country,slug),dplace_geonameid(id,place,slug,lat,lon),occupation(*),occupation_id:occupation,*`
  );
  return res.json();
}
...
export async function generateMetadata({params}, parent) {
  ...
  return {
    ...
    openGraph: {
      images: [
        `https://dev.pantheon.world/api/screenshot/era?id=${id}`,
        ...previousImages,
      ],
    },
  };
