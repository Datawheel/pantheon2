  ...
  const onCheckClick = () => {
    ...
    newBoard[attempt.get()].map((cell, i) => {
      if (selectedPersons.get()[i].id === sortedPersons[i].id) {
        ...
        cells[N_PERSONS * attempt.get() + i + N_PERSONS].className = "card correct";
        resultToShare.set(resultToShare.get() + "🟩");
      }
      else {
        correctPersons.push(false);
        cells[N_PERSONS * attempt.get() + i + N_PERSONS].className = "card wrong";
        resultToShare.set(resultToShare.get() + "🟥");
      }
    });
            ...
            ? persons.length > 0
              ? <div className="panel">
                <ul className="panel-list">
                  {
                    persons.map(person =>
                      <Person data={person} onClick={onPersonClick} isBoardItem={false}/>
                    )
                  }
                </ul>
              </div>
                    ...
                    <ul className="board-row-list">
                      {
                        row.map((cell, j) => {
                          if (cell.person === null) {
                            return <li className="board-row-list-item" key={j}>
                              <div className="card board-item"></div>
                            </li>;
                          }
                          else {
                            return <Person data={cell.person} isBoardItem={true}/>;
                          }
                        })
                      }
                    </ul>
