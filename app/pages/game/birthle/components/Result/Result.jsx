...
export default function Result({MAX_ATTEMPTS, sortedPersons, attempt, isWin, resultToShare, resultBlockRef}) {
        ...
        <div className="result-block-border">
          ...
          <div>
            <ul className="result-links-list">
              {
                sortedPersons.map(person => <li key={person.id}>
          ...
          <div className="btn-list">
            {
              isWin.get()
                ? <div>
                  <CopyToClipboard text={`Pantheon ${attempt.get() + 1}/${MAX_ATTEMPTS}\n\n${resultToShare.get()}`} onCopy={onShareBtnCLick}>
                    <button className="btn">
                      <span className="btn-share" ref={shareBtn}>Share</span>
                    </button>
                  </CopyToClipboard>
                </div>
                : <></>
