    ...
    return (
      <div>
        <Helmet
          ...
          )} | Famous ${toTitleCase(plural(occupation.occupation))} from ${
            country.country
          }`}
          meta={config.meta.map((meta) => {
            if (meta.property && meta.property === "og:title") {
              return {
                property: "og:title",
                content: `Greatest ${country.demonym} ${toTitleCase(
                  plural(occupation.occupation)
                )} Of All Time`,
              };
            }
            if (meta.property && meta.property === "og:description") {
              return {
                ...
                content: `A list of the best ${country.demonym} ${toTitleCase(
                  plural(occupation.occupation)
                )} including a ranking of the top 10 most famous ${toTitleCase(
                  plural(occupation.occupation)
...
const allOccupationsInCountryURL =
  "/occupation_country?country=eq.<country.id>&order=num_people.desc.nullslast";
const allCountriesInOccupationURL =
  "/occupation_country?occupation=eq.<occupation.occupation>&order=num_people.desc.nullslast";
const peopleURL =
  "/person?occupation=eq.<occupation.id>&bplace_country=eq.<country.id>&order=hpi.desc.nullslast&select=bplace_geonameid(id,place,slug),bplace_country(id,continent,country,slug),dplace_country(id,continent,country,slug),dplace_geonameid(id,place,slug),occupation(id,occupation,domain,num_born,hpi,l,occupation_slug,domain_slug),occupation_id:occupation,name,slug,id,hpi,gender,birthyear,deathyear,alive,hpi_prev,l";
const occsInDomainURL =
  "/occupation?domain_slug=eq.<occupation.domain_slug>&select=id";
const wikiSummariesURL =
  "https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&pageids=<people.top10Ids>&origin=*";
...
OccupationCountry.preneed = [
  fetchData(
    "occupation",
    occupationURL,
    (res) => res[0] || { id: null, domain_slug: null }
  ),
  fetchData(
    "country",
    countryURL,
...
OccupationCountry.need = [
  ...
  fetchData(
    "allOccupationsInCountry",
    allOccupationsInCountryURL,
    (res) => res
  ),
  fetchData(
    "allCountriesInOccupation",
    allCountriesInOccupationURL,
    (res) => res
  ),
];
