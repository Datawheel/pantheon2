...
class SelectOccupationCountry extends Component {
  ...
  setOccupation = e => {
    ...
    api(env).get(`/occupation_country?occupation_slug=eq.${e.target.value}&order=num_people.desc.nullslast`)
      .then(occupationCountryResults => {
        ...
        if (countries.length) {
          const chosenCountryInList = countries.filter(d => d.country_slug === selection2).length;
  ...
  render() {
    ...
    const numPeopleSum = data.occupationsInCountry.reduce((a, b) => a + (b.num_people || 0), 0);
    let occupationsInCountry = data.occupationsInCountry.map(d => {
      const rca = d.num_people / d.num_people_country / (d.num_people_occupation / numPeopleSum);
      return {...d, rca};
    });
    ...
    return (
      <div className="welcome">
        <Helmet
          ...
          meta={config.meta.map(meta => {
            if (meta.property && meta.property === "og:title") {
              return {property: "og:title", content: "Occupation country profile selection"};
            }
            if (meta.property && meta.property === "og:description") {
              return {property: "og:description", content: "Select a country and occupation to view a dynamic profile of the greatest people in that selection of all time."};
            }
            return meta;
          })}
        />
        ...
        <div className="welcome-intro">
          <h1 className="welcome-intro-heading u-font-lg u-margin-top-off" aria-label="Select an occupation and country">
        ...
        <div className="welcome-form-outer">
          <div className="welcome-form-inner">
            <h2 className="u-visually-hidden">
              Please select an occupation and country combination to see the most memorable biographies
            </h2>
            ...
            <form onSubmit={e => this.handleSubmit(e)} className="welcome-form">
              ...
              <div className="welcome-form-select-wrapper">
                ...
                >
                  ...
                  {occupations.map(occupation =>
                    <option value={occupation.occupation_slug} key={occupation.occupation_slug}>
              ...
              <div className="welcome-form-button-wrapper">
                <FancyButton
                  ...
                  onClick={() => router.push(`/profile/occupation/${selection1}/country/${selection2}`)}>
                  Go to profile
                </FancyButton>
              </div>
        ...
        <div className="sample-ctr-occs">
          <h2 className="welcome-intro-heading u-font-lg u-margin-top-off" aria-label="The most famous occupation / country combinations">
              ...
              <li key={aCountry.country.slug}>
                <ul className="sample-ctr-occs">
                  {aCountry.values.map(occupationInCountry =>
                    <li key={`${occupationInCountry.country_slug}-${occupationInCountry.occupation_slug}`}>
                      <a href={`/profile/occupation/${occupationInCountry.occupation_slug}/country/${occupationInCountry.country_slug}`}>
                        <img className="country-flag" src={`/images/icons/country/${aCountry.country.slug}.svg`} />
                        <span>{aCountry.country.demonym} {toTitleCase(plural(occupationInCountry.occupation))}</span>
                      </a>
                    </li>
                  )}
                </ul>
