...
const YearRanking = ({ person, ranking }) => {
  ...
  if(birthyearRanking.betterPeers.length){
    betterBirthPeers = <span>Before { person.gender ? "her" : "him" } are: {<AnchorList items={birthyearRanking.betterPeers} name={(d) => `${d.name} (${d.birthcountry.country_code.toUpperCase()})`} url={(d) => `/profile/person/${d.slug}/`} />}. </span>
  }
  if(birthyearRanking.worsePeers.length){
    worseBirthPeers = <span>After { person.gender ? "her" : "him" } are: {<AnchorList items={birthyearRanking.worsePeers} name={(d) => `${d.name} (${d.birthcountry.country_code.toUpperCase()})`} url={(d) => `/profile/person/${d.slug}/`} />}.</span>
  }
  ...
  if(ranking.deathyearPeers.length){
    ...
    if(deathyearRanking.betterPeers.length){
      betterDeathPeers = <span>Before { person.gender ? "her" : "him" } are: {<AnchorList items={deathyearRanking.betterPeers} name={(d) => `${d.name} (${d.deathcountry.country_code.toUpperCase()})`} url={(d) => `/profile/person/${d.slug}/`} />}. </span>
    }
    if(deathyearRanking.worsePeers.length){
      worseDeathPeers = <span>After { person.gender ? "her" : "him" } are: {<AnchorList items={deathyearRanking.worsePeers} name={(d) => `${d.name} (${d.deathcountry.country_code.toUpperCase()})`} url={(d) => `/profile/person/${d.slug}/`} />}.</span>
    }
  }
