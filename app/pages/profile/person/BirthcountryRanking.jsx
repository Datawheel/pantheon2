...
const BirthcountryRanking = ({ person, ranking }) => {
  ...
  if(ranking.betterPeers.length){
    betterPeers = <span>Before { person.gender ? "her" : "him" } are: {<AnchorList items={ranking.betterPeers} name={(d) => `${d.person.name} (${d.person.birthyear})`} url={(d) => `/profile/person/${d.person.slug}/`} />}. </span>
  }
  if(ranking.worsePeers.length){
    worsePeers = <span>After { person.gender ? "her" : "him" } are: {<AnchorList items={ranking.worsePeers} name={(d) => `${d.person.name} (${d.person.birthyear})`} url={(d) => `/profile/person/${d.person.slug}/`} />}.</span>
  }
  ...
  return (
    <div>
      <p>
        Among people born in {person.birthcountry.name}, {person.name} ranks {ranking.me.rank_unique} out of {person.birthcountry.num_born}.&nbsp;
        { betterPeers }
        { worsePeers }
      </p>
      <div>
