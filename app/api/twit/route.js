...
export async function GET(request) {
  ...
  const searchParamPid = searchParams.get("pid");
  if (!searchParamPid) return Response.json([]);
  const uid = parseInt(searchParamPid, 10);
  ...
  // return Response.json(timelineResp);
  ...
  const retTimeline = timelineResp.map(d => {
    ...
    if (d.is_quote_status && d.quoted_status) {
      quote = {
        ...
        created_at:
          d.quoted_status.retweeted_status &&
          d.quoted_status.retweeted_status.created_at
            ? d.quoted_status.retweeted_status.created_at
            : d.quoted_status.created_at,
        id:
          d.quoted_status.retweeted_status &&
          d.quoted_status.retweeted_status.id
            ? d.quoted_status.retweeted_status.id
            : d.quoted_status.id,
        text:
          d.quoted_status.retweeted_status &&
          d.quoted_status.retweeted_status.full_text
            ? d.quoted_status.retweeted_status.full_text
            : d.quoted_status.full_text,
        user: {
          name:
            d.quoted_status.retweeted_status &&
            d.quoted_status.retweeted_status.user &&
            d.quoted_status.retweeted_status.user.name
              ? d.quoted_status.retweeted_status.user.name
              : d.quoted_status.user.name,
          screen_name:
            d.quoted_status.retweeted_status &&
            d.quoted_status.retweeted_status.user &&
            d.quoted_status.retweeted_status.user.screen_name
              ? d.quoted_status.retweeted_status.user.screen_name
              : d.quoted_status.user.screen_name,
          profile_image_url_https:
            d.quoted_status.retweeted_status &&
            d.quoted_status.retweeted_status.user &&
            d.quoted_status.retweeted_status.user.profile_image_url_https
              ? d.retweeted_status.user.profile_image_url_https
              : d.quoted_status.user.profile_image_url_https,
        },
      };
    }
    return {
      ...
      created_at:
        d.retweeted_status && d.retweeted_status.created_at
          ? d.retweeted_status.created_at
          : d.created_at,
      id:
        d.retweeted_status && d.retweeted_status.id
          ? d.retweeted_status.id
          : d.id,
      text:
        d.retweeted_status && d.retweeted_status.full_text
          ? d.retweeted_status.full_text
          : d.full_text,
      user: {
        name:
          d.retweeted_status &&
  ...
  const retUser = {
    ...
    profile_image_url: userResp.profile_image_url_https.replace(
      "_normal",
      "_bigger"
    ),
  };

  return Response.json({user: retUser, timeline: retTimeline});
}
