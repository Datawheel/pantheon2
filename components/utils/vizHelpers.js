...
export const calculateYearBucket = (
  ...
  let a = linspace(0, Math.log10(big), buckets).map((d) =>
    Math.ceil(Math.pow(10, d))
  );
  a = a.filter((d, i) => i === a.indexOf(d));

  const h = histogram().domain([0, big]).thresholds(a)(years).reverse();
  ...
  data.forEach((d) => {
    ...
    d.yearWeight =
      bucket.x1 === bucket.x0
        ...
        : !bucket.x0
        ? 1 / laterYears
        : 1 / (bucket.x1 - bucket.x0);
    d.yearBucket = `${h.indexOf(bucket)}`;
  });

  const labels = h.map((b) =>
    FORMATTERS.year(Math.round(maxYear - (b.x0 + (b.x1 - b.x0) / 2)))
  );
  ...
  const ticks = labels
    ...
    .filter(
      (i) =>
        !i ||
        i === labels.length - 1 ||
        (i <= labels.length - divisions && !(i % divisions))
    );
  ...
  return (g) => (d) => {
    ...
    if (d[g]) val = d[g];
      ...
      if (!attrs[d.occupation_id])
        console.log(
          `No occupation found with ID: ${d.occupation_id}`,
          attrs[d.occupation_id]
        );
...
export const groupTooltip = (data, accessor = () => []) => ({
  body: (d) => {
    ...
    people
      .filter((d, i) => peopleNames.indexOf(d.name) === i)
      .slice(0, 3)
      .forEach((n) => {
        txt += `<br /><span class="bold">${n.name}</span> b.${FORMATTERS.year(
          n.birthyear
        )}`;
      });
    return txt;
...
export const peopleTooltip = {
  body: (d) => {
    const birthyear =
      d.birthyear instanceof Array ? min(d.birthyear) : d.birthyear;
    const deathyear =
      d.deathyear instanceof Array ? max(d.deathyear) : d.deathyear;
    ...
    return deathyear !== null
      ? `<span class="bold">${FORMATTERS.year(birthyear)} - ${FORMATTERS.year(
          deathyear
        )}</span>${age} year${suffix}</span>`
      : `<span class="bold">Born ${FORMATTERS.year(
          birthyear
        )}</span>${age} year${suffix}</span>`;
  },
...
export const toTitleCase = (myStr) =>
  typeof myStr === "string"
    ? myStr.replace(
        /\w\S*/g,
