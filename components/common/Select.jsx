...
export default function Select({
  ...
  return (options && options.length) || children ? (
    <label
      className={`${
        className ? `${className} ` : ""
      }${namespace}-select-label u-font-${fontSize}${
        inline ? " cms-inline-select-label" : ""
      }`}
    >
      {label && (
        <span
          className={`${namespace}-select-text${
            label && labelHidden ? " u-visually-hidden" : ""
          }`}
        >
          {label}
        </span>
      ...
      <select
        ...
        className={`${namespace}-select ${
          disabled ? `${namespace}-select-disabled` : ""
        }`}
        onChange={onChange}
        value={value}
        tabIndex={tabIndex}
